...
describe('General time tests', () => {
	it('adds one hour if we are in day light saving time otherwise returns normal time', () => {
		...
		let dayDst = times['30-3']['isha']; //20:38
		expect(addOneHour(day)).toBe('20:36');
		expect(addOneHour(dayDst, new Date('2026-03-30'))).toBe('21:38');
		expect(addOneHour(dayDst, new Date(2026, 2, 29, 3))).toBe('21:38');
		expect(addOneHour(dayDst, new Date(2026, 2, 29, 1, 59, 59))).toBe('20:38');
	});
	...
	it("provides an object with a given date's prayer times with respect to DST", () => {
		...
		expect(prayersNotDst['fajr']).toBe('04:45');
		expect(prayersNotDst['dhuhr']).toBe('12:22');
		expect(prayersDst['fajr']).toBe('04:33');
